...
def load_pmbot(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"lucifer/plugins/assistant/pmbot/{shortname}.py")
        name = "lucifer.plugins.assistant.pmbot.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["lucifer.plugins.assistant.pmbot." + shortname] = mod
